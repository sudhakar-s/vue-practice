  ...
  <div class="catalogs">
    ...
    <section>
      <div class="d-flex flex-wrap justify-space-between gap-8 align-start">
        <div>
          <Heading size="large" weight="bold">Service Hub</Heading>
          <Text class="mt-5">
            Organize services, manage and track versioning and API service documentation.
            <Link href="https://konghq.com" target="_blank" color="surface.text.skyblue.normal">Learn More</Link>
          </Text>
        </div>
        <div class="catalogs__search-wrapper flex-wrap d-flex gap-6">
          <TextInput v-model="searchValue" :leading-icon="SearchIcon" showClearButton
            accessibility-label="Search service catalogs" placeholder="Search" />
          <Button weight="semibold" :icon="PlusIcon" @click="handleCreate">Service Package</Button>
        </div>
      </div>
    </section>
    <!-- List section -->
    <section class="mt-8">
      <Progress v-if="!isPending && isFetching" accessibility-label="Fetching service catalogs" />
      <Loading v-if="isPending" />
      <Error v-else-if="isError" @retry="handleRetry" />
      <template v-else-if="isSuccess && !!data?.length" >
        <div class="catalogs__cards">
          <Card v-for="catalog in data" :key="catalog.id" :catalog="catalog" />
        </div>
      </template>
      <Empty v-else />
    </section>
